...
export default class SpoUrlShortener extends React.Component<IUrlShortenerProps, {}> 
{
    ...
    private async _spApiPost (url: string, type: string, inputUrl: string, key: string): Promise<object> 
    {
    ...
    private generateRandomHash(hashLength: number) {
    ...
    private async _shorten_url_with_hash () 
    {
        ...
        if (this.state.inputUrl) 
        {
            ...
            {
                // generate Id
                let hash = this.generateRandomHash(this.props.idLength);
                // check if Id exists
            ...
            if (hashIsUnique) {
                // post outputId and input URL to list
                ...
                if (getListResponse["status"] == 200) 
                {
                    ...
                    if (!repeatLimitExceeded) {
                        ...
                        let setItemsResponse = await this._spApiPost(postUrl, type, this.state.inputUrl, outputId);
            ...
            if (this.state.inputId) 
            {
                ...
                if (getItemResponse["status"] == 200) {
                    ...
                    if (item.length == 0)
                    {
                        ...
                        let type = getListResponse["ListItemEntityTypeFullName"];
                        let postUrl = `/_api/web/lists/GetById('${this.props.lookupList.id}')/items`;
                        let setItemsResponse = await this._spApiPost(postUrl, type, this.state.inputUrl, this.state.inputId);
                        if (setItemsResponse["status"] == 201) {
                            let outputUrl = this.properties.currentUrl + "?$id=" + this.state.inputId;
                            this.setState({postFeedback2: true, outputUrl: outputUrl});
                        }
                        else {this.setState({shortenError: `SharePoint API error: ${setItemsResponse["odata.error"].message.value}`});}
                    } 
                    // id exists
    ...
    private async _find_url () 
    {
        ...
        if (findId) {
            // search Id in list
            let getItemResponse = await this._spApiGet(`/_api/web/lists/GetById('${this.props.lookupList.id}')/items?$select=Key,TargetUrl&$filter=Key%20eq%20'`+findId+"'"); 
            let item = getItemResponse["value"];
            if (getItemResponse["status"] == 200) 
            {
                // if findId exists
                if (item.length == 1) {
                    ...
                    this.setState({inputUrl: item[0].TargetUrl.Url, outputUrl: outputUrl, lookUpFeedback: true });
                }
    ...
    public async componentDidMount () 
    {
        if (this.properties.currentUrl.includes("?$id="))
        {
            ...
            // check if Id valid
            let getItemResponse = await this._spApiGet(`/_api/web/lists/GetById('${this.props.lookupList.id}')/items?$select=Key,TargetUrl&$filter=Key%20eq%20'`+idFromUrl+"'"); 
            let item = getItemResponse["value"];
            if (item.length == 1) {
                this.setState({urlIdStatus: 3, redirectingUrl: item[0].TargetUrl.Url, outputId: idFromUrl});
                window.location.href = item[0].TargetUrl.Url;
            }
            // if invalid
