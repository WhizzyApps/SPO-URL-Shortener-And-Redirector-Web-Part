...
export default class UrlShortenerWebPart extends BaseClientSideWebPart<IUrlShortenerWebPartProps> 
{
    ...
    private async _createListField (listName:string, FieldName:string, FieldType:number, EnforceUniqueValues:boolean, required:string) 
    {
        // create Field
    ...
    protected getPropertyPaneConfiguration(): IPropertyPaneConfiguration 
    {
        ...
        {
            const url = `${this.context.pageContext.web.absoluteUrl}/_api/web/lists/GetById('${this.properties.lookupList}')/fields?$select=Title&$filter=((Title eq 'Title') or (Title eq 'Key') or (Title eq 'TargetUrl') or (Title eq 'TargetUrl2')) and Hidden eq false and ReadOnlyField eq false `;
            const response = await this._spApiGet(url);
            if (response["status"] && response["status"].toString().startsWith("2"))
            {
                if (response["value"].length >= 3) 
                {
                    ...
                    ["Title", "Key", "TargetUrl", "TargetUrl2"].forEach(requiredField => {
                    ...
                    if (allRequiredFieldsExist) {
                        alert(`List '${this.properties.newListName}' is valid for use as lookup list. Columns 'Title', 'Key', 'TargetUrl', 'TargetUrl2' exist.`);
                    }
                    else {
                        alert(`List '${this.properties.newListName}' is not valid for use as lookup list. One of the columns 'Title', 'Key', TargetUrl', 'TargetUrl2' is missing or has the wrong type. Please create a new lookup List.`);
                    }
                }
                else {
                    alert(`List '${this.properties.newListName}' is not valid for use as lookup list. One of the columns 'Title', 'Key', TargetUrl', 'TargetUrl2' is missing or has the wrong type. Please create a new lookup List.`);
        ...
        const _stack_to_create_list = async () => 
        {
            ...
            {
                const createListField1Resonse = await this._createListField(this.properties.newListName, "Key", 2, true, "true");
                if (createListField1Resonse["status"].toString().startsWith("2"))
                {
                    const createListField2Resonse = await this._createListField(this.properties.newListName, "TargetUrl", 11, false, "false");
                    if (createListField2Resonse["status"].toString().startsWith("2"))
                    {
                        const createListField3Resonse = await this._createListField(this.properties.newListName, "TargetUrl2", 3, false, "false");
                        if (createListField3Resonse["status"].toString().startsWith("2"))
                        {
                            alert(`List '${this.properties.newListName}' successfully created`);
                            await this._getLists();
                            this.context.propertyPane.refresh();
                        }
                        else {alert(createListField3Resonse["odata.error"].message.value);}
                    }
        ...
        const about = [
            ...
            PropertyPaneLink('link', {  
                href: 'https://sharepoint-url-shortener.com',
                text: 'Documentation',
                target: '_blank',
            }),
        ];
